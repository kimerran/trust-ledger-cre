...
function onHttpTrigger(runtime2, payload) {
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  runtime2.log("TrustLedger Risk Monitor — workflow started");
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  ...
  runtime2.log(`[1/4] Decision ID: ${input.decisionId}`);
  runtime2.log(`[1/4] Model ID:    ${input.modelId}`);
  runtime2.log(`[1/4] Outcome:     ${input.decision.outcome}`);
  runtime2.log(`[1/4] Confidence:  ${input.decision.confidence}`);
  ...
  runtime2.log(`[1/4] Canonical payload length: ${canonical.length} chars`);
  runtime2.log(`[1/4] Hash: ${hash}`);
  runtime2.log("[2/4] Signing with AWS KMS...");
  const kmsKeyArn = getSecretSafe(runtime2, "KMS_KEY_ARN");
  const awsKeyId = getSecretSafe(runtime2, "AWS_ACCESS_KEY_ID");
  const awsSecret = getSecretSafe(runtime2, "AWS_SECRET_ACCESS_KEY");
  ...
  if (!isSimulation) {
    ...
    const kmsResult = runtime2.runInNodeMode((nodeRuntime, body, keyId, secret, endpoint) => {
      const sender = new cre.capabilities.HTTPClient;
      const response = sender.sendRequest(nodeRuntime, {
        url: endpoint,
        method: "POST",
        headers: {
    ...
    }, { aggregation: "MODE" })(kmsBody, awsKeyId, awsSecret, runtime2.config.kmsEndpoint).result();
    signature = String(kmsResult);
  }
  runtime2.log(`[2/4] Signature: ${signature.substring(0, 32)}...`);
  runtime2.log("[3/4] Assessing risk with Claude Haiku...");
  ...
  if (hasAnthropicKey) {
    const claudeBody = JSON.stringify({
      model: runtime2.config.claudeModel,
      max_tokens: 256,
      system: 'You are a risk assessment AI for financial and regulated AI decisions. Always respond with valid JSON only — no markdown, no explanation. Schema: {"riskLevel": "LOW"|"MEDIUM"|"HIGH", "summary": "<1-2 sentences>"}',
      messages: [
        {
          role: "user",
          content: [
...
`)
    ...
    const riskResult = runtime2.runInNodeMode((nodeRuntime, body, apiKey) => {
      const sender = new cre.capabilities.HTTPClient;
      const response = sender.sendRequest(nodeRuntime, {
    ...
    try {
      riskAssessment = JSON.parse(String(riskResult));
    } catch (error) {
      runtime2.log(`llm error: ${error}`);
      runtime2.log("[WARN] Failed to parse Claude response");
    }
  }
  ...
  runtime2.runInNodeMode((nodeRuntime, body, apiKey, url) => {
    const sender = new cre.capabilities.HTTPClient;
    const response = sender.sendRequest(nodeRuntime, {
      url,
      method: "POST",
    ...
    if (!ok(response)) {
      ...
      runtime2.log(`[4/4] ✓ Callback delivered (${response.statusCode})`);
    }
    return response.statusCode;
  }, { aggregation: "MODE" })(callbackBody, internalKey, callbackUrl).result();
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  runtime2.log("TrustLedger Risk Monitor — complete");
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
